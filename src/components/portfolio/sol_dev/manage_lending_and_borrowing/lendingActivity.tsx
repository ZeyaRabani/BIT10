...
export default function LendingActivity() {
    ...
    const fetchRecentLendingActivity = async (address: string) => {
        const response = await userRecentLendActivity({ source_chain: 'Solana', address: address });
        ...
        queries: [
            {
                ...
                queryFn: () => wallet.publicKey ? fetchRecentLendingActivity((wallet.publicKey).toString()) : toast.error('User address is undefined')
            }
        ]
    });
