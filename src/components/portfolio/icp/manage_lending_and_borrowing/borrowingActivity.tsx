...
export default function BorrowingActivity() {
    ...
    const fetchRecentBorrowingActivity = async (address: string) => {
        const response = await userRecentBorrowActivity({ source_chain: 'ICP', address: address });
        ...
        queries: [
            {
                ...
                queryFn: () => ICPAddress ? fetchRecentBorrowingActivity(ICPAddress) : toast.error('User address is undefined')
            }
        ]
    });
