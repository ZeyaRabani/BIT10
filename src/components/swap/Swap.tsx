...
import { ChevronsUpDown, Loader2, Settings, MoveLeft, Wallet, ArrowUpDown, Info } from 'lucide-react'
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip'
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion'
import { fetchSepoliaTokenBalance, createSepoliaTransaction } from './eth_sepolia/ETHSepoliaSwapModule'
import { fetchBscTestnetTokenBalance, createBscTestnetTransaction } from './bsc_testnet/BSCTestnetSwapModule'
...
export default function Swap() {
    ...
    const bnbAmount = useMemo(() => payWithPriceQueries[2].data, [payWithPriceQueries]);
    const btcAmount = useMemo(() => payWithPriceQueries[3].data, [payWithPriceQueries]);
    ...
    const fromBalanceQueries = useQueries({
        queries: [
            ...
            {
                queryKey: ['ethSepoliaBalance', address, '0x0000000000000000000000000000000000000000e'],
                queryFn: () => {
            ...
            // For BSC Testnet
            {
                queryKey: ['bscTestnetBnbBalance', address, '0x0000000000000000000000000000000000000000b'],
                queryFn: () => {
                    if (!address || chain !== 'bsc_testnet') return '0';
                    return fetchBscTestnetTokenBalance('0x0000000000000000000000000000000000000000b', address);
                },
                enabled: !!address && chain === 'bsc_testnet',
                refetchInterval: 10000, // 10 seconds
            },
            {
                queryKey: ['bscTestnetUsdcBalance', address, '0x64544969ed7EBf5f083679233325356EbE738930'],
                queryFn: () => {
                    if (!address || chain !== 'bsc_testnet') return '0';
                    return fetchBscTestnetTokenBalance('0x64544969ed7EBf5f083679233325356EbE738930', address);
                },
                enabled: !!address && chain === 'bsc_testnet',
                refetchInterval: 10000, // 10 seconds
            },
            {
                queryKey: ['bscTestnetBtcbBalance', address, '0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8'],
                queryFn: () => {
                    if (!address || chain !== 'bsc_testnet') return '0';
                    return fetchBscTestnetTokenBalance('0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8', address);
                },
                enabled: !!address && chain === 'bsc_testnet',
                refetchInterval: 10000, // 10 seconds
            },
    ...
    const fetchTokenBalance = useMemo(() => {
        ...
        // For Ethereum Sepolia
        if (sourceChain === 'Ethereum' && chain === 'eth_sepolia' && selectedFromToken.address === '0x0000000000000000000000000000000000000000e') {
            return fromBalanceQueries[0].data ?? '0';
        } else if (sourceChain === 'Ethereum' && chain === 'eth_sepolia' && selectedFromToken.address === '0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238') {
            return fromBalanceQueries[1].data ?? '0';
        }
        // For BSC Testnet
        else if (sourceChain === 'Binance Smart Chain' && selectedFromToken.address === '0x0000000000000000000000000000000000000000b') {
            return fromBalanceQueries[2].data ?? '0';
        } else if (sourceChain === 'Binance Smart Chain' && selectedFromToken.address === '0x64544969ed7EBf5f083679233325356EbE738930') {
            return fromBalanceQueries[3].data ?? '0';
        } else if (sourceChain === 'Binance Smart Chain' && selectedFromToken.address === '0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8') {
            return fromBalanceQueries[4].data ?? '0';
        }
        else {
            return '0';
        }
    ...
    const getTokenPrice = useCallback((tokenId: string) => {
        ...
        } else if (token.label === 'tBNB') {
            return parseFloat(bnbAmount ?? '0');
        } else if (token.label === 'BTCB') {
            return parseFloat(btcAmount ?? '0');
        }
        return 0;
    }, [bnbAmount, btcAmount, ethAmount, usdcAmount]);
    ...
    const swapDisabledConditions = !chain || swaping || selectedFromToken?.token_id === selectedToToken?.token_id || !matchingPool || chain !== fromTokenChain || form.watch('from_amount') >= (Number(fetchTokenBalance) * 1.01) || form.watch('from_amount') <= 0 || form.watch('to_amount') <= 0;
    ...
    return (
        <div className='flex flex-col items-center justify-center py-4'>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <Input className='dark:border-white' placeholder='Recipient wallet address' {...field} />
                                                        </FormControl>
                                                        <FormDescription>
            ...
            {chain &&
                <div className='flex items-start w-[330px] md:w-[610px]'>
                    <VerifyTransaction />
                </div>
            }
        </div>
