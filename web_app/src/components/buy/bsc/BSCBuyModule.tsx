...
export const verifyBSCTransaction = async ({ mode, trxHash }: { mode: string, trxHash: string }) => {
    try {
        const host = 'https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io';
        const canisterId = '6phs7-6yaaa-aaaap-qpvoq-cai';
        ...
        if (mode === 'buy') {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //  @ts-expect-error
            transfer = await actor.bsc_buy(trxHash);
        } else {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //  @ts-expect-error
            transfer = await actor.bsc_sell(trxHash);
        }
