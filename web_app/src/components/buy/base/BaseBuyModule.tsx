...
export const sellPayTokensBase = [
    { label: 'BIT10.TOP', value: 'BIT10.TOP', img: BIT10Img as StaticImageData, address: '0x2d309c7c5FbBf74372EdfC25B10842a7237b92dE', tokenType: 'ERC20', slug: ['top crypto'] },
]
...
export const sellBaseBIT10Token = async ({ tokenInAddress, tokenOutAddress, tokenOutAmount, tokenInAmount, baseAddress }: { tokenInAddress: string, tokenOutAddress: string, tokenOutAmount: string, tokenInAmount: string, baseAddress: string }) => {
    try {
        const host = 'https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io';
        const canisterId = '6phs7-6yaaa-aaaap-qpvoq-cai';
        ...
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        //  @ts-expect-error
        const transfer = await actor.base_sell(txResponse.hash);
...
export const verifyBaseTransaction = async ({ mode, trxHash }: { mode: string, trxHash: string }) => {
    try {
        const host = 'https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io';
        const canisterId = '6phs7-6yaaa-aaaap-qpvoq-cai';
        ...
        if (mode === 'buy') {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //  @ts-expect-error
            transfer = await actor.base_buy(trxHash);
        } else {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //  @ts-expect-error
            transfer = await actor.base_sell(trxHash);
        }
