...
export type LeaderboardTableDataType = {
    position: number,
    address: string,
    total_points: number
}
...
interface DataTableProps<TData> {
    columns: ColumnDef<LeaderboardTableDataType>[];
    data: TData[];
...
export function DataTable<TData, TValue>({
    ...
    userSearchColumn,
    inputPlaceHolder,
}: DataTableProps<LeaderboardTableDataType>) {
    ...
    const renderCellContent = (cell: CellContext<LeaderboardTableDataType, unknown>, row: { original: LeaderboardTableDataType }) => {
        switch (cell.column.id) {
            case 'user_rank':
                return (
                    <div>
                        {row.original.total_points > 20 ? '🏆 Grandmaster'
                            : row.original.total_points > 15 ? '🎖 Master'
                                : row.original.total_points > 12 ? '🥇 Gold'
                                    : row.original.total_points > 11 ? '🥈 Silver'
                                        : '🥉 Bronze'}
                    </div>
                );
            default:
