...
export type PortfolioTableDataType = {
    ...
    transaction_timestamp: bigint | number;
    network: string;
}
        ...
        switch (cell.column.id) {
            ...
            case 'tickIn':
                return (
                    <div className='flex flex-row space-x-1 items-center'>
                        <div>{formatCompactNumber(parseFloat(row.original.token_in_amount))}</div>
            ...
            case 'tickOutName':
                return (
                    <div className='flex flex-row space-x-1 items-center'>
                        <div>
                            {row.original.transaction_type === 'Swap'
                                ? row.original.token_out_amount
                                : formatCompactNumber(parseFloat(row.original.token_out_amount))}
                        </div>
            ...
            case 'viewTransaction':
                return (
                    <a
                        href={`${getTokenExplorer(row.original.token_out_address)}${getTokenExplorer(row.original.token_out_address).endsWith('/') ? '' : '/'}${row.original.network == 'ICP' ? row.original.swap_id : row.original.token_out_tx_hash}`}
                        target='_blank'
