...
export default function BalanceAndAllocation() {
    ...
    const balanceAndPriceQueriesConfig = useMemo((): UseQueryOptions[] => {
        ...
        return queries;
    }, [chain, isICPConnected, isEVMConnected, isSolanaConnected, icpAddress, evmAddress, publicKey, fetchBIT10Tokens]);
    ...
    const bit10TOPTokensResponse = balanceAndPriceQueries[0]?.data as BIT10TokensResponse;
    const bit10TOPTokens: BIT10Token[] = bit10TOPTokensResponse?.data ?? [];
    const bit10TOPPrice = bit10TOPTokensResponse?.tokenPrice ?? 0;
    ...
    const icpBIT10DEFITokenBalance = icpQueryIndex >= 0 ? (balanceAndPriceQueries[icpQueryIndex]?.data as bigint | undefined) : undefined;
    const icpBIT10TOPTokenBalance = icpQueryIndex >= 0 ? (balanceAndPriceQueries[icpQueryIndex + 1]?.data as | bigint | undefined) : undefined;
    const baseBIT10TOPTokenBalance = baseQueryIndex >= 0 ? (balanceAndPriceQueries[baseQueryIndex]?.data as string | undefined) : undefined;
    const solanaBIT10TOPTokenBalance = solanaQueryIndex >= 0 ? (balanceAndPriceQueries[solanaQueryIndex]?.data as string | undefined) : undefined;
    const bscBIT10TOPTokenBalance = bscQueryIndex >= 0 ? (balanceAndPriceQueries[bscQueryIndex]?.data as string | undefined) : undefined;
    ...
    const totalTokens = () => {
        if (chain === 'icp' && isICPConnected) {
            ...
            return baseBIT10TOPTokenBalance;
        } else if (chain === 'solana' && isSolanaConnected) {
            return solanaBIT10TOPTokenBalance;
        } else if (chain === 'bsc' && isEVMConnected) {
            return bscBIT10TOPTokenBalance;
        } else {
            return 0;
        }
    };
    ...
    const bit10DEFI = () => {
        if (chain === 'icp' && isICPConnected) {
            return icpBIT10DEFITokenBalance;
        } else {
            return 0;
        }
    };
    ...
    const bit10TOP = () => {
        if (chain === 'icp' && isICPConnected) {
            return icpBIT10TOPTokenBalance;
        } else if (chain === 'base' && isEVMConnected) {
            return baseBIT10TOPTokenBalance;
        } else if (chain === 'solana' && isSolanaConnected) {
            return solanaBIT10TOPTokenBalance;
    ...
    const tokenData = [
        ...
        // }
        ...(chain === 'icp' && Number(bit10DEFITokenBalance) > 0
            ? [{
                token: 'BIT10.DEFI',
                balance: formatCompactNumber(Number(bit10DEFITokenBalance))
            }]
