...
type TestBit10MEMERebalanceData = {
    timestmpz: string;
...
const jsonFilePath = path.join(__dirname, '../../../data/test_bit10_meme_rebalance.json');
const cache = new NodeCache({ stdTTL: 1800 });

const fetchAndUpdateData = async () => {
    try {
        const priceOfTokenToBuy = 20;
        const historicalDataPath = path.join(__dirname, '../../../data/bit10_meme_historical_data.json');

        const historicalData = await readJsonFile(historicalDataPath) as {
            bit10_meme_historical_data: {
                timestmpz: string;
                tokenPrice: number;
                data: Token[];
            }[];
        ...
        const latestData = historicalData.bit10_meme_historical_data[0];
        ...
        const existingData = await readJsonFile(jsonFilePath) as { test_bit10_meme_rebalance: TestBit10MEMERebalanceData[] };

        const latestRebalance = existingData.test_bit10_meme_rebalance[0]?.newTokens || [];
        ...
        cache.set('bit10_meme_rebalance_data', existingData);

        await writeJsonFile(jsonFilePath, existingData);

        console.log('Adding rebalance data for BIT10.MEME');
    } catch (error) {
        console.error('Error fetching rebalance data for BIT10.MEME:', error);
    }
}

// cron.schedule('*/30 * * * * *', () => { // 30 sec
cron.schedule('0 10 * * 5', () => { // Every Friday at 3:30 PM IST
    fetchAndUpdateData().catch(error => console.error('Error in fetchAndUpdateData for BIT10.MEME Rebalance:', error));
});

export const handleTestBit10MEMERebalance = async (request: IncomingMessage, response: ServerResponse) => {
    if (request.method !== 'GET') {
        response.setHeader('Content-Type', 'application/json');
        response.writeHead(405);
        response.end(JSON.stringify({ error: 'Method Not Allowed for BIT10.MEME Rebalance Data' }));
        return;
    }

    try {
        let existingData = cache.get<{ test_bit10_meme_rebalance: TestBit10MEMERebalanceData[] }>('bit10_meme_rebalance_data');
...
async function readJsonFile(filePath: string) {
    try {
        ...
        return { test_bit10_meme_rebalance: [] };
    }
}
