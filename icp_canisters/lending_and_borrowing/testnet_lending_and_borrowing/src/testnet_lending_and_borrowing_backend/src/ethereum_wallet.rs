...
use ic_secp256k1::{PublicKey, RecoveryId, DerivationIndex, DerivationPath};
...
impl EthereumWallet {
    ...
    fn compute_recovery_id(&self, message_hash: &[u8], signature: &[u8]) -> RecoveryId {
        ...
        assert!(
            ...
            "Failed to verify signature prehashed for Ethereum, digest: {:?}, signature: {:?}, public_key: {:?}",
...
fn derive_public_key(owner: &Principal, public_key: &EcdsaPublicKey) -> EcdsaPublicKey {
    let derivation_path = DerivationPath::new(
        derivation_path_for_principal(owner)
            .into_iter()
            .map(DerivationIndex)
            .collect(),
    );
...
fn derivation_path_for_principal(owner: &Principal) -> Vec<Vec<u8>> {
