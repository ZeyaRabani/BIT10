use crate::ecdsa::ecdsa_key::EcdsaPublicKey;
...
impl EthereumWallet {
    ...
    pub async fn sign_with_ecdsa(&self, message_hash: [u8; 32]) -> Result<([u8; 64], RecoveryId), String> {
        ...
        let signature = <[u8; 64]>::try_from(result.signature).map_err(|_| {
            format!(
                "invalid signature from management canister. Expected 64 bytes but got {} bytes",
                signature_length
            )
        })?;
    ...
    fn compute_recovery_id(&self, message_hash: &[u8], signature: &[u8]) -> Result<RecoveryId, String> {
